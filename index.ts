...
function gitPush() {
  return new Promise(async (resolve: (value: boolean) => void) => {
    Deno.env.set("GIT_DIR", "/github/workspace/data/mborejdo/.git/");
    const add = run({
      cmd: ["git", "add", "."],
    });
    const addResult = await add.status();
    const commit = run({
      cmd: ["git", "commit", "-a", "-m", "Update Repo"],
    });
...
function patchReadme() {
  return new Promise(async (resolve: (value: boolean) => void) => {
    await writeTextFile("/github/workspace/data/mborejdo/README.md", "Hello World!" + Math.random());
    resolve(true);
  });
}
...
await Promise.all([
  gitConfigure(),
  gitClone(`https://${API_TOKEN_GITHUB}@github.com/mborejdo/mborejdo.git`, "/github/workspace/data/mborejdo")
]);
await patchReadme();
await gitPush();